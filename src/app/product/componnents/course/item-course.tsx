export default function ItemCourse({ isTime = false, isReverse = false, src, price }: { isTime?: boolean; isReverse?: boolean; src?: string; price?: string }) {
  const Content = () => (
        ...
        <div>
          ...
          <div className="space-y-4 md:text-[0.875rem] text-[0.85rem] ">
            ...
            <div className="flex items-start gap-2">
              <img src="icons/check.png" className="w-[1.25rem] h-[1.25rem] mt-1" />
              <div>
                Giúp con Rèn giũa phẩm chất bản thân mỗi ngày với giáo trình thực chứng đã được áp dụng thành công cho cộng đồng Tâm Trí Lực và các thành viên đội tuyển Siêu trí
                nhớ, Siêu trí tuệ Việt Nam trong hơn 10 năm qua.
              </div>
            </div>
            <div className="flex items-start gap-2">
              <img src="icons/check.png" className="w-[1.25rem] h-[1.25rem] mt-1" />
              <div>Cùng con rèn luyện thể lực với hành trình Hiền tài khỏe mạnh.</div>
            </div>
          </div>
        </div>
