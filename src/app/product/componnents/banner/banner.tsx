...
export default function HitaGameSection() {
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      const information1 = document.querySelector(".information1") as HTMLElement;
      const information2 = document.querySelector(".information2") as HTMLElement;
      ...
      if (banner && information1 && information2) {
        ...
        setSliderHeight(bannerHeight + informationHeight1 + informationHeight2 + 90);
      }
    };
    handleResize();
  ...
  return (
    <section className="w-full flex flex-col relative" style={{ backgroundImage: "url('bg-banner.png')", backgroundSize: "cover" }}>
      <div
        ...
        <Slider {...settings}>
          {images.map((src, index) => (
            <div key={index} className="relative w-full">
              <img src={src} alt={`Slide ${index + 1}`} className="w-full md:rounded-xl h-[250px] md:h-[400px]" />
            </div>
          ))}
        </Slider>
      </div>
      <div
        style={{ height: `${sliderHeight - bannerHeight / 3}px`, marginTop: `${bannerHeight / 3}px` }}
      ...
      >
        <div className=" information1 w-full grid grid-cols-1 md:grid-cols-2 gap-x-8 items-center justify-end mt-auto max-w-[100%] md:max-w-[768px] xl:max-w-[80%] lg:max-w-[80%]">
          <div className=" md:mx-0 mx-[1rem]">
            ...
            <h2 className="md:text-[2rem] text-[1.5rem] font-bold">HITA GAME - Trò chơi của những hiền tài</h2>
          </div>
          <div className=" md:mx-0 mx-[1rem]  ">
        ...
        <div className="information2 w-[95%] mb-[2rem] py-4 md:mx-[1rem] grid lg:border lg:border-[#FFFFFF1F] grid-cols-2 lg:grid-cols-4 gap-4 text-white rounded-[1rem] lg:bg-gradient-to-b lg:from-transparent lg:to-[rgba(255,255,255,0.06)]">
          ...
          <div className="flex flex-col items-center  lg:p-4 rounded-lg justify-center gap-2">
            ...
            <p className="lg:text-sm text-[0.85rem]">Phong cách, Biến & Token</p>
            <p className="lg:text-[2rem] md:text-[2rem] text-[1.1rem] font-bold">200+</p>
          </div>
        </div>
      </div>
      <div
        className="bg-[#272727] bg-gradient-to-b from-transparent to-white md:mx-[1rem] h-[20px] "
      ></div>
    </section>
  );
}
